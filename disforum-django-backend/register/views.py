...
from register.serializers import RegistrationSerializer, ClassDataSerializer, PasswordChangeSerializer, Class_assoSerializer, RegisterClassDataSerializer, StudentSerializer
...
class ClassDataList(APIView):
    ...
    def post(self, request, format=None):
        ...
        serializer = RegisterClassDataSerializer(data=q_dict)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
